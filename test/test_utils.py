...
def test_continuous_discrete_equivalence_EDM_sigma_schedule():
  num_steps = 1000
  sigma_min = 0.002
  sigma_max = 80
  rho = 7
  ...
  ts, dt = get_times(num_steps)
  sigma = get_EDM_sigma_function(sigma_min=sigma_min, sigma_max=sigma_max)
  ...
  for i, (t_cur, t_next) in enumerate(
    zip(expected_discrete_sigmas[:-1], expected_discrete_sigmas[1:])
  ):  # 0, ..., N-1
    print(t_cur, t_next)
    x_cur = x_next
...
test_continuous_discrete_equivalence_EDM_sigma_schedule()
