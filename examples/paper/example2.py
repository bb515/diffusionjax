...
def main():
    ...
    # score_model, params, opt_state, mean_losses = retrain_nn(
    #     n_batch, m_0, C_0, N_epochs, step_rng, score_model, params, opt_state,  # 2000
    #     loss_function, decomposition=decomposition)
    ...
    if decomposition:
        ...
        fx = eval_steps(train_ts[100:])

        fig, ax = plt.subplots(1)
        ax.set_title("Loss")
        ax.plot(train_ts[100:], fx[:], label=r"$\hat{L}(\theta)$")
        ax.set_ylabel("Loss")
        ax.set_xlabel(r"$t$")
        plt.savefig("losses_t00.png")
        plt.close()
        ...
        #fx0true, fx1true = eval_steps_true(train_ts)
        fxapproxtrue = eval_steps_approx_true(train_ts[100:])
        ...
        fig, ax = plt.subplots(1)
        ax.plot(train_ts[100:], fxtrue, label=r"$L(\theta)$")
        ax.set_ylabel("Loss")
        ax.set_xlabel(r"$t$")
        # ax.set_xscale("log")
        # ax.set_yscale("log")
        plt.legend()
        plt.savefig("losses_t01.png")
        plt.close()

        fig, ax = plt.subplots(1)
        ax.plot(train_ts[100:], fxapproxtrue, label=r"$\tilde L(\theta)$")
        ax.set_ylabel("Loss")
        ax.set_xlabel(r"$t$")
        # ax.set_xscale("log")
        # ax.set_yscale("log")
        plt.legend()
        plt.savefig("losses_t02.png")
        plt.close()

        fig, ax = plt.subplots(1)
        ax.set_title("Difference in loss, {:d} samples vs {:d} samples".format(J_true, J))
        ax.plot(train_ts[100:], (-fx + fxtrue), label=r"$|L(\theta) - \hat{L}(\theta)|$")
        ax.set_ylabel("Loss component")
        ax.set_xlabel(r"$t$")
        # ax.set_xscale("log")
        # ax.set_yscale("log")
        ...
        fig, ax = plt.subplots(1)
        ax.plot(train_ts[100:],  (-fx + fxapproxtrue), label=r"$|L(\theta) - \hat{L}(\theta)|$")
        ax.set_ylabel("Loss component")
        ax.set_xlabel(r"$t$")
        # ax.set_xscale("log")
        # ax.set_yscale("log")
