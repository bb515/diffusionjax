...
def sample_image(num_samples, kernel, N=16):
    """Samples from a GMRF
    """
    x = np.linspace(-3, 3, N)
    y = np.linspace(-3, 3, N)
    ...
    print(np.shape(z))
    C_0 = kernel(z)
    z = random.normal(xx.shape[0])
    x = solve_triangular(cholesky(C_0), z, lower=True)
...
def main():
    sample_image(num_samples=1, kernel=Matern32())
    assert 0
    num_epochs = 4000
